...
public class ODConcurrentValidator extends ODValidator{
    ...
    public List<ODCandidate> chooseODs(List<ODCandidate> ods){
    ...
    protected ODValidationResult validateOneOdCandidate(DataFrame data, ODTree.ODTreeNode node,
                                                        VisitCountConsideredMap<ODTree.ODTreeNode,ODTreeNodeEquivalenceClasses> cache,
                                                        HashMap<ODTree.ODTreeNode, ODTree.ODTreeNode> nodeForNodeToGetCache,
                                                        ConcurrentHashMap<ODTree.ODTreeNode, ODTree.ODTreeNodeStatus> cutOD
    ){
        if(!node.accessible()){
        ...
        ODTreeNodeEquivalenceClasses odTreeNodeEquivalenceClasses=
                cache.get(node);
        if(odTreeNodeEquivalenceClasses != null) {
            if (cache.containKey(node)) {
                odTreeNodeEquivalenceClasses = odTreeNodeEquivalenceClasses.deepClone();
            }
        }
        else{
            odTreeNodeEquivalenceClasses = new ODTreeNodeEquivalenceClasses();
        }
        while(!path.isEmpty()){
            node=path.pop();
            ...
            if(!node.confirm){
                ...
                if(result.status!=node.status){
                    ...
                    updateCacheVisitCountBeforeNodeChildrenCut(node,cache,nodeForNodeToGetCache);
                    node.concurrentSetStatus(result.status,cutOD);
    ...
    public Set<Integer> validatebyods(List<ODCandidate> notConfirmedODs, DataFrame data, boolean isVioRowsEmpty, ConcurrentHashMap<ODTree.ODTreeNode, ODTree.ODTreeNodeStatus> cutOD) {
        VisitCountConsideredMap<ODTree.ODTreeNode, ODTreeNodeEquivalenceClasses> cache = new VisitCountConsideredMap<>();
        ...
        cache.put(notConfirmedODs.get(0).odByODTreeNode.getRoot(),new ODTreeNodeEquivalenceClasses());

        for(ODCandidate od:notConfirmedODs){
            ODValidationResult result= validateOneOdCandidate
                    (data,od.odByODTreeNode,cache,nodeForNodeToGetCache,cutOD);
